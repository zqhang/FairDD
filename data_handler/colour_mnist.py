...
class BiasedMNIST(MNIST):
    ...
    def _make_biased_mnist(self, indices, label, is_foreground):
    ...
    def build_biased_mnist(self):
        ...
        for bias_label, indices in bias_indices.items():
            _data, _targets = self._make_biased_mnist(indices, bias_label, self.is_foreground)
...
def get_biased_mnist_dataloader(root, batch_size, data_label_correlation,is_foreground,
                                n_confusing_labels=9, train=True, num_workers=8):
    transform = transforms.Compose([
        transforms.ToTensor(),
        transforms.Normalize(mean=(0.5, 0.5, 0.5),
                             std=(0.5, 0.5, 0.5))])
    dataset = ColourBiasedMNIST(root+"/MNIST", train=train, transform=transform,
